function [ZCoutput]=functionrs3(N,c,Alpha,G,seed)
...
Is=eye(N);
for casers=1:3
disp(['run into functionrs3-R3-，accepting--[ ',num2str(casers),' ]---++++++']);
% casers=1;
%%%----设定初始变量----------------------end----------%  
...
%    Pr=(P0*one'-one'*(((Is-Yita*rs)^-1)-Is)')*((Betas-G.*Ss)^-1)*(Alpha-P0*one);  
