           ...
           for j=1:size(Sstep,2)
            ...
            XR1i(x)=0;
               ...
               for k=1:length(temp)     
                   key=Ss(:,temp(k)).*XR1i;
                   key(key<c)=0;
                   if c>Yita*XR1(temp(k))+sum((Yita/2)*(key)) || error>rand(1)
%                    if (c>Yita*XR1(temp(k))+sum((Yita/2)*(key)) && (1-error)>rand(1)) || (c<=Yita*XR1(temp(k))+sum((Yita/2)*(key)) && error>rand(1))   
                       Ss(temp(k),x)=0;
                     [Sstep,Ssr]=dedges(Ss,Sstep,Ssr,x,temp(k));
%                        disp([num2str(temp(k)),'去除：',num2str(Yita*XR1(temp(k)))]);
                    fprintf(fid,'%s\r\n',[num2str(temp(k)),'去除：',num2str(Yita*XR1(temp(k)))]);
                   else
%                        disp([num2str(temp(k)),'保留：',num2str(Yita*XR1(temp(k)))]);
                       fprintf(fid,'%s\r\n',[num2str(temp(k)),'保留：',num2str(Yita*XR1(temp(k)))]);
                   end
               end
           end
       end
