function [Yita,P0]=TheStepOners3(Alpha,G,Ss,~,rs,seed,casers)
N=length(G);
one=ones(N,1);
I=eye(N);
Beta=N*I;
...
%-----------计算Yita范围----------------------------
Yita=sym('Yita');
y1=(one'*((Betas-Gs.*Ss)^(-1))*Alphas+one'*(((Is-Yita*rs)^-1)-Is)'*((Betas-Gs.*Ss)^-1)*one)/(2*one'*((Betas-Gs.*Ss)^-1)*one)-p1;
y2=(one'*((Betas-Gs.*Ss)^(-1))*Alphas+one'*(((Is-Yita*rs)^-1)-Is)'*((Betas-Gs.*Ss)^-1)*one)/(2*one'*((Betas-Gs.*Ss)^-1)*one)-p2;
Yita1=vpa(solve(y1));
Yita2=vpa(solve(y2));
...
if Yita2>=0             %当 Yita范围与 大于零 有交集时
    ...
    %MATLAB的运算
    Yita=sym('Yita');
    Pr1_Yita=(one'*((Betas-Gs.*Ss)^(-1))*Alphas+one'*(((Is-Yita*rs)^-1)-Is)'*((Betas-Gs.*Ss)^-1)*one)/(2*one'*((Betas-Gs.*Ss)^-1)*one);
    pr=(Pr1_Yita*one'-one'*(((Is-Yita*rs)^-1)-Is)')*((Betas-Gs.*Ss)^-1)*(Alphas-Pr1_Yita*one);
    dy=diff(pr);
    rz=solve(dy==0,Yita);
...
%     rz=max(rz);
    ...
    Yita1=rz;
    pr1=(one'*((Betas-Gs.*Ss)^(-1))*Alphas+one'*(((Is-Yita1*rs)^-1)-Is)'*((Betas-Gs.*Ss)^-1)*one)/(2*one'*((Betas-Gs.*Ss)^-1)*one);
